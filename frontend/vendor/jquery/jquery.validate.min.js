...
jQuery.validator.addMethod("accept",function(a,b,c){var d=typeof c==="string"?c.replace(/,/g,"|"):"image/*",e=this.optional(b),f,g;if(e){return e}if($(b).attr("type")==="file"){d=d.replace("*",".*");if(b.files&&b.files.length){for(f=0;f<b.files.length;f++){g=b.files[f];if(!g.type.match(new RegExp(".?("+d+")$","i"))){return false}}}}return true},jQuery.format("Please enter a value with a valid mimetype."))
